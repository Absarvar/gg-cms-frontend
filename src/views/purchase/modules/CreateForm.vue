  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item label="源头">
          <a-space>
            <a-select
              defaultActiveFirst
              ref="select"
              style="width: 250px"
              v-decorator="['sourceId', {initialValue:37 ,rules:[{required: true, message: '请选择源头'}]}]"
              :options="sourceOptions.butcherEntList"
            ></a-select>
          </a-space>
          <!-- <a-input v-decorator="['sourceId', {rules:[{required: true, message: '请输入源头id'}]}]" /> -->
        </a-form-item>
        ...
        <a-form-item label="预计到货时间"><a-input v-decorator="['planArrive', {initialValue:'晚上8-10点' ,rules:[{required: true, message: '请输入预计到货时间'}]}]" /></a-form-item>
        <a-form-item hidden label="订货日期"><a-input v-decorator="['orderDate', {}]" /></a-form-item>
        <a-form-item hidden label="回货日期"><a-input v-decorator="['backDate', {}]" /></a-form-item>
...
import { goodsOptions, goodsUnitOptions, sourceOptions } from '@/api/commonData'
...
export default {
  ...
  methods: {
    ...
    onChangePrice (e) {
      const number = Number(this.form.getFieldValue('num'))
      const specification = Number(this.form.getFieldValue('sku'))
    ...
    setDefaultVal () {
      ...
      this.form.setFieldsValue({ 'orderDate': date * 1 })
      this.form.setFieldsValue({ 'backDate': date * 1 })
