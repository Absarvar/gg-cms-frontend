<template>
  ...
  >
    ...
    <a-card style="margin-top: 24px" :bordered="false" title="现场信息">
      <a-card type="inner" title="散货信息" style="width:300px;float:left;">

        <a-descriptions title="散货选择" size="small" :col="1">
          <a-descriptions-item label="">
            <a-radio-group v-model="goodsId" button-style="solid" >
              <template v-for="(opt) in specGoodsOptions" :label="opt">
                <template>
                  <a-radio-button style="margin:5px" :key="opt.value" :value="opt.value">
                    {{ opt.label }}
                  </a-radio-button>
                </template>
              </template>
        ...
        <a-descriptions title="框编号" size="small" :col="2">
          <a-descriptions-item label="">
            ...
            <a-tag id="orbitCodeLabel" style="height:30px;font-size: 35px;" color="orange" >{{ this.orbitCode }}</a-tag>
...
export default {
      ...
      columns: [
        {
          ...
          width: 75,
          resizable: 'true',
          scopedSlots: { customRender: 'orbitCode' }
        },
        {
          title: '框数',
          dataIndex: 'num',
  ...
  methods: {
    ...
    handleAdd () {
      ...
      this.product.goodsId = this.goodsId
      this.product.applyId = this.info.id

      this.product.weightFur = this.weightFur
